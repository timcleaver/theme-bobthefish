...
function __bobthefish_prompt_aws_vault_profile -S -d 'Show AWS Vault profile'
    ...
    [ -z "$AWS_SESSION_EXPIRATION" -a -z "$AWS_CREDENTIAL_EXPIRATION" ]
    or return
    ...
    if [ -n  "$AWS_CREDENTIAL_EXPIRATION" ]
        set expiry (gdate -d "$AWS_CREDENTIAL_EXPIRATION" +%s)
    else
...
function __bobthefish_prompt_awsume_profile -S -d 'Show AWSume profile'
    ...
    set -l now (date --utc +%s)
    set -l expiry (date -d "$AWSUME_EXPIRATION" --utc +%s)
    set -l diff_mins (math "floor(( $expiry - $now ) / 60)")
...
function __bobthefish_prompt_aws_region -S -d 'Show AWS region'
    [ "$theme_display_aws_region" = 'yes' ]
    or return

    [ -n "$AWS_REGION" ]
    or return

    # print globe then aws
    set -l segment \uf0ac " " $AWS_REGION " " \ue7ad
    set -l status_color $color_aws_region
...
function __bobthefish_prompt_node -S -d 'Display current node version'
    ...
    if type -q nvm
      ...
      set node_manager 'nodenv'
      set node_manager_dir $NODENV_ROOT
      [ -n "$node_manager_dir" ]
        or return
      set -l node_version ("$node_manager" version-name  2> /dev/null)
    end
